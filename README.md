...
终端顶部搜索框支持按股票名称或六码代码查询深圳主板股票，范围仅限 `000`、`001`、`002` 开头的代码。搜索候选来自当前腾讯公开行情通道，并在腾讯服务和 Next.js 服务端进行两次市场范围过滤。

搜索到的非核心股票会打开“研究模式”详情页：可展示真实报价、日线数据能力、来源和更新时间，但不参与 20 只核心观察池的评分、交易信号、交易计划、报告排行或批量刷新。上游报价或日线不可用时，页面会明确提示，不会使用 Mock 数据伪装为真实行情。
