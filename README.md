...
产品边界：不接入券商、不自动下单，因此券商级专业行情接口与券商交易能力不在排期范围内。
...
分钟线缺失时，尾盘趋势策略自动切换为**收盘后日线确认**模式：
用当日涨幅 2.5%-6% + 当日量不低于 20 日均量 1.1 倍在收盘后确认信号，
不再因拿不到分钟线而阻断（策略输出会标注"收盘后日线确认"）。
...
演示模式支持深市主板任意 6 位代码（`000`/`001`/`002`/`003` 开头）。
观察池之外的深市主板代码会自动生成确定性的演示行情画像；
沪市、科创板、创业板不在支持范围内。默认观察池仍为 20 只核心股票。
...
- 访问 `/backtest` 可运行六种策略的历史回测：**突破买入**、**均线交叉**、**趋势波段**、
  **趋势波段（过滤版）**（MA60 上行 + 动量>5% + 收盘距 MA20≤6% + 量能 1-2.5 倍均量）、
  **龙头首阴修复**（生产策略信号，日线可还原）、**尾盘趋势·日线近似**（涨幅 2.5%-6% + 量能放大）。
- 回测数据使用腾讯财经历史日线（每个请求最多 500 根 K 线），信号由确定性策略引擎生成。
- 龙头首阴修复支持首阴出现在启动后 3 个交易日内（`maxDaysAfterLaunch`），修复在首阴后 2 日内确认。
- 尾盘趋势的日线近似因历史接口不含换手率，未使用 5%-15% 换手区间，仅用涨幅与量能。
...
## 批量策略研究 `npm run signal:study`

- 拉取深市主板股票的历史日线，对**龙头首阴修复、尾盘趋势（收盘确认）、趋势波段**三套生产策略，
  统计信号后 1/3/5/10 个交易日的平均收益与胜率，并与"全样本持有"基准对比。
- 20 只观察池 × 500 交易日实测：趋势波段过滤版将信号数从 870 收敛到 136，
  10 日超额收益由 -1.12% 改善为 +1.16%，胜率 54.4%；过滤条件已同步到 `/backtest` 的策略选项。
- 用法：`npm run signal:study`（观察池 20 只）或 `npm run signal:study 002472 002317`（指定股票）。
- 前置：本地腾讯行情服务（`:8001`）已启动。
