...
默认技术计算使用 `qfq`。`syncDailyBars` 先查询本地 `qfq` 覆盖数量；已有不少于 250 个交易日时不重复拉取完整历史。覆盖不足或强制检查时，才调用行情服务拉取完整历史。

上游失败时保留本地最近成功日线，返回 `stale`，不把旧日线标为最新。

## 4. 分钟线能力与限制

分钟线通过本地数据仓库和同步流程提供，当前保留 1m、5m、15m、30m、60m Provider 接口。
...
第一版保存的板块快照使用行情公开报价对观察池映射股票做核心方向代理评分，source 明确标记来源，不得作为完整 A 级机会依据。
...
- 实时报价使用腾讯公开行情接口（qt.gtimg.cn）。
- 行业/概念板块数据当前只做观察池代理评分，不冒充完整板块统计。
