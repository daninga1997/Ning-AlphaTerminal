...
- 节假日列表当前为手动配置，后续可通过AKShare `tool_trade_date_hist_sina` 接口获取正式交易日历
- 板块数据当前使用AKShare公开报价对观察池核心方向做代理评分，标记为partial，不得作为完整A级机会依据
- 分钟线数据优先使用AKShare `stock_zh_a_hist_min_em`，失败时只读取本地真实stale数据，不使用日线推测分钟线
- 当前网络环境无法访问东方财富实时接口（`stock_zh_a_spot_em`），使用新浪作为唯一可用源

## 12. 后续正式交易日历接入方案

1. 使用AKShare `tool_trade_date_hist_sina` 获取历史交易日历
